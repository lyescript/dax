var loading63 = typeof(host47global) === "undefined";
var _e12;
  ...
  for (k in _o5) {
    ...
    if (! number63(_k4)) {
      ...
      if (quasisplice63(v, depth)) {
        _e25 = quasiexpand(v[1]);
      } else {
        _e25 = quasiexpand(v, depth);
      }
      var _v1 = _e25;
      last(xs)[_k4] = _v1;
    }
...
var op_delims = function (parent, child) {
  ...
  if (_e33(precedence(_child), precedence(_parent))) {
    return(["(", ")"]);
  } else {
    return(["", ""]);
  }
...
compile = function (form) {
  ...
  if (nil63(_form)) {
    ...
    if (special_form63(_form)) {
      ...
      var _e38;
      if (atom63(_form)) {
        _e38 = compile_atom(_form);
      } else {
        var _e39;
        if (infix63(hd(_form))) {
          _e39 = compile_infix(_form);
        } else {
          _e39 = compile_call(_form);
        }
        _e38 = _e39;
      }
      var _form1 = _e38;
...
var lower_if = function (args, hoist, stmt63, tail63) {
  ...
  if (stmt63) {
    var _e41;
    if (is63(_else)) {
      _e41 = [lower_body([_else], tail63)];
    }
    return(add(hoist, join(["%if", lower(cond, hoist), lower_body([_then], tail63)], _e41)));
  } else {
    var e = unique("e");
    add(hoist, ["%local", e]);
    var _e40;
    if (is63(_else)) {
      _e40 = [lower(["%set", e, _else])];
    }
    add(hoist, join(["%if", lower(cond, hoist), lower(["%set", e, _then])], _e40));
    return(e);
  }
};
var lower_short = function (x, args, hoist) {
  ...
  if (some63(hoist1)) {
    ...
    var _e42;
    if (x === "and") {
      _e42 = ["%if", _id18, b, _id18];
    } else {
      _e42 = ["%if", _id18, _id18, b];
    }
    return(lower(["do", ["%local", _id18, a], _e42], hoist));
  } else {
...
var lower_while = function (args, hoist) {
  ...
  var _e43;
  if (none63(pre)) {
    _e43 = ["while", _c, lower_body(body)];
  } else {
    _e43 = ["while", true, join(["do"], pre, [["%if", ["not", _c], ["break"]], lower_body(body)])];
  }
...
var fix_require = function () {
  var _id26 = (function () {
    try {
      module.filename = require("path").resolve("repl");
      return([true, module.filename]);
    }
    catch (_e55) {
      return([false, _e55]);
    }
  ...
  var ok = _id26[0];
  if (! ok) {
    var dir = require("path").dirname(process.execPath);
    module.filename = require("path").resolve(dir, "repl");
  }
...
setenv("%if", {_stash: true, special: function (cond, cons, alt) {
  ...
  indent_level = indent_level + 1;
  var _x121 = compile(cons, {_stash: true, stmt: true});
  indent_level = indent_level - 1;
  var _cons1 = _x121;
  var _e44;
  if (alt) {
    indent_level = indent_level + 1;
    var _x122 = compile(alt, {_stash: true, stmt: true});
    indent_level = indent_level - 1;
    _e44 = _x122;
  }
  var _alt1 = _e44;
...
setenv("while", {_stash: true, special: function (cond, form) {
  ...
  indent_level = indent_level + 1;
  var _x124 = compile(form, {_stash: true, stmt: true});
  indent_level = indent_level - 1;
  var body = _x124;
  var ind = indentation();
  if (target === "js") {
...
setenv("%for", {_stash: true, special: function (t, k, form) {
  ...
  var ind = indentation();
  indent_level = indent_level + 1;
  var _x126 = compile(form, {_stash: true, stmt: true});
  indent_level = indent_level - 1;
  var body = _x126;
  if (target === "lua") {
...
setenv("%try", {_stash: true, special: function (form) {
  ...
  var ind = indentation();
  indent_level = indent_level + 1;
  var _x131 = compile(form, {_stash: true, stmt: true});
  indent_level = indent_level - 1;
  var body = _x131;
  var hf = ["return", ["%array", false, e]];
  indent_level = indent_level + 1;
  var _x134 = compile(hf, {_stash: true, stmt: true});
  indent_level = indent_level - 1;
  var h = _x134;
  return(ind + "try {\n" + body + ind + "}\n" + ind + "catch (" + e + ") {\n" + h + ind + "}\n");
}, stmt: true, tr: true});
...
setenv("%object", {_stash: true, special: function () {
  ...
  for (k in _o11) {
    ...
    if (number63(_k11)) {
      ...
      var _v3 = _id30[1];
      if (! string63(_k12)) {
        throw new Error("Illegal key: " + str(_k12));
      }
      s = s + c + key(_k12) + sep + compile(_v3);
      c = ", ";
    }
  }
  return(s + "}");

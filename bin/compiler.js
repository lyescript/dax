  ...
  for (k in _o5) {
    ...
    if (! number63(_k4)) {
      ...
      if (quasisplice63(v, depth)) {
        _e22 = quasiexpand(v[1]);
      } else {
        _e22 = quasiexpand(v, depth);
      }
      var _v1 = _e22;
      last(xs)[_k4] = _v1;
    }
...
var op_delims = function (parent, child) {
  ...
  if (_e30(precedence(_child), precedence(_parent))) {
    return(["(", ")"]);
  } else {
    return(["", ""]);
  }
...
var lower_if = function (args, hoist, stmt63, tail63) {
  ...
  if (stmt63) {
    var _e38;
    if (is63(_else)) {
      _e38 = [lower_body([_else], tail63)];
    }
    return(add(hoist, join(["%if", lower(cond, hoist), lower_body([_then], tail63)], _e38)));
  } else {
    var e = unique("e");
    add(hoist, ["%local", e]);
    var _e37;
    if (is63(_else)) {
      _e37 = [lower(["%set", e, _else])];
    }
    add(hoist, join(["%if", lower(cond, hoist), lower(["%set", e, _then])], _e37));
    return(e);
  }
};
var lower_short = function (x, args, hoist) {
  ...
  if (some63(hoist1)) {
    ...
    var _e39;
    if (x === "and") {
      _e39 = ["%if", _id18, b, _id18];
    } else {
      _e39 = ["%if", _id18, _id18, b];
    }
    return(lower(["do", ["%local", _id18, a], _e39], hoist));
  } else {
...
var lower_while = function (args, hoist) {
  ...
  var _e40;
  if (none63(pre)) {
    _e40 = ["while", _c, lower_body(body)];
  } else {
    _e40 = ["while", true, join(["do"], pre, [["%if", ["not", _c], ["break"]], lower_body(body)])];
  }
  return(add(hoist, _e40));
};
...
setenv("%try", {_stash: true, special: function (form) {
  ...
  indent_level = indent_level + 1;
  var _x133 = compile(hf, {_stash: true, stmt: true});
  indent_level = indent_level - 1;
  var h = _x133;
  return(ind + "try {\n" + body + ind + "}\n" + ind + "catch (" + e + ") {\n" + h + ind + "}\n");
}, stmt: true, tr: true});
...
setenv("error", {_stash: true, special: function (x) {
  var _e43;
  if (target === "js") {
    _e43 = "throw " + compile(["new", ["Error", x]]);
  } else {
    _e43 = "error(" + compile(x) + ")";
  }
  var e = _e43;
